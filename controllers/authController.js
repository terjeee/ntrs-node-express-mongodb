...
const { signJWT } = require("../utils/jasonwebtoken");
...
exports.signUp = async (req, res, next) => {
  try {
    ...
    const jwtoken = signJWT(String(newUser._id));
...
exports.signIn = async (req, res, next) => {
  try {
    ...
    const tokenJWT = signJWT(dbUser._id);
...
exports.resetPassword = async function (req, res, next) {
  try {
    ...
    const tokenJWT = signJWT(user._id);
...
exports.protectWithLogIn = async function (req, res, next) {
  try {
    ...
    const jwtVerified = jsonwebtoken.verify(jwt, process.env.JWT_KEY);

    // CHECK IF USER EXISTS with userID on the JWT variable (jwtVerified)
    const user = await ModelUser.findById(jwtVerified.id);

    if (!user) return next(new AppError("User doesn't exist.", 401));

    // CHECK IF USER CHANGED PASSWRD AFTER TOKEN WAS ISSUED
    if (user.isPasswordChangedAfter(jwtVerified.iat))
